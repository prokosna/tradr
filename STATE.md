...
Fourteen DCRs have been raised since design "finished". Six of them fixed defects that would have been expensive after implementation: an `aud` compared as a scalar, a dependency diagram that inverted the architecture, sender-chosen strings used as path components, identity-key signatures with no domain separation — that one let a Brokr impersonate a device — a layout table that put a Layer 1 trait in a Layer 3 crate, and a filename able to disguise its own extension.
