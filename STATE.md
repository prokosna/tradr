...
1. **WI-M0-011f**, fetching the JWKS bytes over HTTP. The last piece of docs/05 step 2 that does not exist, and the one that can be finished without waiting on anything: Google's JWKS is public, so this is verifiable against the real endpoint with no OAuth flow behind it. **Open decision 17 must be settled first**, since it asks which HTTP client and whether the trait is async
2. **WI-M0-008**, Google OAuth on desktop, loopback with PKCE. M0 is done when two devices exchange Attestations and verify each other, and that needs real tokens. It also has to lift the client secret out of the two gitignored downloads in the repository root and delete them -- **those two files are the only copy on this machine**
3. **WI-M0-007b**, persisting keys through the OS key store: Linux Secret Service, Android Keystore. This is where `backing()` stops being a constant
