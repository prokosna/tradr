...
Seventeen DCRs have been raised since design "finished". Six of them fixed defects that would have been expensive after implementation: an `aud` compared as a scalar, a dependency diagram that inverted the architecture, sender-chosen strings used as path components, identity-key signatures with no domain separation — that one let a Brokr impersonate a device — a layout table that put a Layer 1 trait in a Layer 3 crate, and a filename able to disguise its own extension.
...
2. **WI-M0-007**, key generation and OS key store storage. **Settle first whether it is a Critical Module**: STATE.md's Work Item table says yes, [CLAUDE.md](CLAUDE.md) §6's table does not list it, and the two must agree before the Work Order goes out, because the answer decides who writes the tests
