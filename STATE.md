...
| 17 | **Which HTTP client fetches a JWKS, and whether the trait that wraps it is async.** DCR-022 kept the fetch out of `tradr-identity` deliberately, so this is a free choice rather than a constrained one -- but it decides whether `BoxFuture` (ADR-0013) reaches a second family of traits or whether this joins `KeyStore` in DF-9's synchronous group. A blocking network call is not the same tradeoff as a blocking local signature: it is seconds, and it can hang. Settle it with a real client in hand rather than on paper | Before WI-M0-011f | Supervisor |
