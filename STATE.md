...
## Next three actions
1. **WI-M5-008, the one blocker left**: no file can be sent from Windows at all (F-W1), so the M5 verification cannot finish. Everything before the transfer -- sign-in, discovery, the static entry, the dial, the pin -- is confirmed working on the real pair
