...
## Next three actions
1. **`WI-M6-005b`, the Invite's codec and its generator.** `WI-M6-005a` landed the message and the Layer 0 type, so what is left is the blob -- the version byte, the base64url, the 4096-character cap -- and `create_invite` drawing its two random halves through `Rng` and its expiry from a `Clock`. **The QR is not in it and was in the row it replaces**: DCR-071 settles that the QR encodes exactly the blob string, so rendering one is the interface's job and belongs to `WI-M6-007`. What that Work Item inherits is a number rather than a sketch -- a blob is about 1.7 KB, QR byte mode holds 2953 bytes at its lowest error-correction level, and an invite too large for a QR must go by the paste channel and be told so
2. **`WI-M6-006`, the exchange in the plugin**: the window an invite opens, the reply, the approval, and both sides storing the Link. **It is the first caller `LinkRegistry` has ever had**, and DCR-070's signatures are what stop it forgetting the secret half -- but note the observation review left on it: `add`'s rollback unlinks a slot unconditionally, so two `LinkRegistry` instances over one file could have a stale one's failed `add` delete a slot the live one's record still names. The composition root holds one registry or it inherits that
...
**`WI-M6-001` through `WI-M6-004b` and `WI-M6-005a` have landed; the rest are not cut yet.** They are the shape M6 is expected to take, listed so the order is on record; each is cut properly when its turn comes, and any of them may change then. **Two of the four were re-cut at dispatch and the table records why in its own rows**, which is the reason a row is worth more than the order it sits in.
...
| WI-M6-005b | The Invite, second half: the blob codec in `tradr-proto` -- the wire conversion, the leading version byte, unpadded base64url and the 4096-character cap -- and `create_invite` in `tradr-identity`, which draws the `InviteId` and the `HalfSecret` through `Rng` and sets `expires_at` from a `Clock`. **The version byte sits in front of the protobuf body rather than inside it**, because a version field inside the payload could only be read after deciding the bytes are protobuf, which is the decision the version exists to make. `link.rs`'s codec is the precedent for the two error types | planned | |
