...
1. **WI-M0-011c**, the Attestation **issue** side. `expected_nonce` is one private function in `attestation.rs` and `classify` is its only caller; issuing means exposing that same computation, never writing a second one. Critical Module: **the whole security of step 4 is that both sides compute one thing**, and two implementations that agree today are two that can drift apart later. The test that matters is a mint-then-classify round trip, and the mutation that matters is whether editing the issue side breaks the verify suite too
