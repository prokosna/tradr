...
## Next three actions
1. **`WI-M6-006`, the exchange in the plugin**: the window an invite opens, the reply, the approval, and both sides storing the Link. **Everything it composes now exists** -- `create_invite` and the blob codec from `WI-M6-005b`, the three framed messages from `WI-M6-004a`/`004b`, and a `LinkRegistry` that takes the secret store as an argument so a caller cannot store a Link and forget its secret. **It is the first caller `LinkRegistry` has ever had**, and the observation review left on it stands: `add`'s rollback unlinks a slot unconditionally, so two `LinkRegistry` instances over one file could have a stale one's failed `add` delete a slot the live one's record still names. The composition root holds one registry or it inherits that. **DCR-067 is what it has to implement and the part with no precedent**: a Control stream whose first frame is a `LinkReply` rather than a `Hello`, carrying no session and no Trust Tier, accepted only while an invite is open and closed at the first completed exchange
2. **`WI-M6-007`, the interface**: show the QR, scan or paste one, show both Fingerprints, approve or decline, list Links and remove one. **The QR moved here from `WI-M6-005`'s original row and DCR-071 is why**: the QR encodes exactly the blob string `invite_to_blob` returns, so rendering one is a display concern and not a codec one. **What it inherits is a number rather than a sketch** -- a blob runs to about 1.7 KB and QR byte mode holds 2953 bytes at its lowest error-correction level, so an invite too large for a QR must go by the paste channel, and the interface says which of the two it is faced with rather than rendering a QR that will not scan. F-W3 is the standing warning about this milestone's interface work: a thing that reaches the list and cannot be acted on where a user looks for it is an affordance failure that no test sees
...
### A Work Order asserted a dependency's behaviour from that dependency's own comment, found 2026-09-02

**`WI-M6-003b`'s Work Order told the Implementer that `secret_service`'s `Item::delete` calls an `ensure_unlocked` that prompts rather than fails, and it does not.** `blocking/item.rs:45-51` in the pinned 5.1.0 returns `Err(Error::Locked)`. The claim came from the crate's own comment at `item.rs:89` -- "ensure_unlocked handles prompt for unlocking if necessary" -- **which contradicts the function three lines below it.** The Implementer wrote the claim into a comment, faithfully, and review caught it by reading the crate.

**This is the `pkg-config` entry and the `agy` entry a third time, and the first time in a Work Order.** Both of those record the same act: asserting a capability instead of probing for one, and the assertion outliving whatever made it true. Here the assertion was never true. **The instrument that exists is the one that found it** -- the review read the dependency's source rather than the Work Order's account of it -- and what is worth writing down is that a Work Order is now a place this failure lands, not only a document.

**The line it justified is load-bearing for a reason nobody had written down**, which is what made the false comment dangerous rather than merely wrong. A locked collection has not said whether it holds a slot, so `remove` without `ensure_unlocked` would iterate an empty search result and report a slot emptied that was never touched. The comment is deleted rather than corrected: `store` and `load` make the same call and comment neither.
...
**`WI-M6-001` through `WI-M6-005b` have landed; the rest are not cut yet.** They are the shape M6 is expected to take, listed so the order is on record; each is cut properly when its turn comes, and any of them may change then. **Two of the four were re-cut at dispatch and the table records why in its own rows**, which is the reason a row is worth more than the order it sits in.
