...
Eleven DCRs have been raised since design "finished". Five of them fixed defects that would have been expensive after implementation: an `aud` compared as a scalar, a dependency diagram that inverted the architecture, sender-chosen strings used as path components, identity-key signatures with no domain separation — that one let a Brokr impersonate a device — and a layout table that put a Layer 1 trait in a Layer 3 crate.

## Next three actions

1. **WI-M0-006d**, the `Vfs` trait. DCR-011 has landed, so [ADR-0014](docs/adr/0014-vfs-exposes-operations-never-paths.md) is in `docs/` and the Work Item can be cut against it. Fix the method set for the write half first — the ADR deliberately left it open
2. **WI-M0-006e**, the `Transport` trait, split from WI-M0-006d because the two carry separate decisions. Its stream model is the least settled thing in M0; scope it to what [docs/03](docs/03-discovery-and-transport.md) actually pins down and say in the Work Order what is being left to M1
...
| WI-M0-006d | Layer 1 trait: `Vfs`. **Blocked on the DCR-011 docs commit**, which carries [ADR-0014](docs/adr/0014-vfs-exposes-operations-never-paths.md) | todo | |
