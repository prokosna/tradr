...
**F-W3. A registered Static Peer could not be selected, and it is not yet clear whether that is a defect or an affordance.** The Static Peers section lists entries with a Remove button and no way to act on one; the design has the entry appear in the **Discovered Peers** list instead, as an unidentified peer, which is where it can be selected and sent to. That section's own empty text says "No peers discovered on local network yet", which tells a reader a hand-registered peer does not belong there. **So even where the plumbing works the interface hides it.** Whether the observation reaches the peer list at all is the open question, and one look settles it -- see the Next three actions.

## Next three actions
1. **WI-M5-008 and WI-M5-006, which together block the M5 verification**: a file cannot be sent from Windows at all (F-W1), and the file picker is dead (F-W2). WI-M5-007's sign-in defect is worked around for today by a clock resync, so it no longer blocks
1b. **One observation settles F-W3 and only the user can make it**: after adding a Static Peer, does an entry appear in the **Discovered Peers** list, labelled either with the typed label or "Unidentified peer"? If yes, the plumbing works and the fault is the interface. If no, the observation is not reaching the peer list and that is a defect in the source or the drain
...
| WI-M5-008 | **`is_absolute` in place of `starts_with('/')`** (F-W1), and a test that a Windows-shaped absolute path is recognised. **Sending a file from Windows is impossible until this lands** | todo | |
