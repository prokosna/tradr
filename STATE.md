...
1. **WI-M0-011e**, the JWKS cache. `parse_jwks` turns bytes into keys; this decides **when a device may go and get new bytes**, which is the half with the denial-of-service question in it. Critical Module, so the tests are written first. The design is settled: the cache returns a decision and the composition root does the I/O, so `tradr-identity` stays synchronous and pure, and **returning `Refetch` is itself what spends the budget**, which is why `lookup` takes `&mut self` -- a caller that loops gets `Unknown` on the second call however it is written
...
| WI-M0-011e | **The JWKS cache**: offline verification against it, and **at most one rate-limited refetch** on an unknown `kid`, since random `kid` values would otherwise be a denial-of-service primitive (DCR-020) | todo | Yes |
