...
|---|---|---|---|
| WI-M0-013c | PASS | 0 | Narrowing the one gate that was failing on schedule. The distinction it now draws is between `STATE.md` in the working tree and `git show HEAD:STATE.md`: a `DCR-N` in the first and not the second is one being introduced right now, and everything else is judged exactly as before. **The case worth building was the one where both are true at once** -- the tree adding `DCR-024` while `DCR-019` sits at `HEAD` named in no commit message -- because the tempting shortcut is to exempt a dirty `STATE.md` wholesale, and that shortcut passes every other test. It reports `DCR-019` and never mentions `DCR-024`. I built all five cases in scratch repositories rather than reading the diff. **The invariant is unchanged, only its timing**: a DCR added and then committed under a message that never names it is caught on the next run instead of the same one |
...
| WI-M0-013c | **Stop `ci/state-sync.sh` Check 3 failing on the DCR it is watching being born.** A DCR that the working tree is adding to `STATE.md` is exempt; one already committed to `STATE.md` and still in no commit message is not | **done** -- PASS, no REVISE | Yes |
