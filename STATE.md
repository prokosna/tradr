...
## Next three actions
1. **`WI-M6-006`, the exchange in the plugin**: the window an invite opens, the reply, the approval, and both sides storing the Link. **`WI-M6-003b` is what it was waiting on** -- once a Link and its secret land and leave together, the plugin has one call to make rather than two to remember, which is the whole reason DCR-070 put the store in the registry's signature rather than in the plugin's
...
**`WI-M6-003b`, on `wi-m6-003b-link-secret-slot`.** DCR-070 landed docs-first: `SecretStore` gains `remove`, and `LinkRegistry::add` and `remove` take the secret store as an argument so that discarding the Link Secret is not a thing a caller can forget. **The Supervisor tests are written first** -- section 6, since `add` and `remove` are the Critical Module the registry is one for -- and per DF-30 they are rustfmt-clean and through `sh ci/run-all.sh` before dispatch this time.
