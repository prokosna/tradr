...
## Next three actions
1. **`WI-M6-005`, the invite: generated, rendered as a QR and as a base64 blob, parsed back, and expired.** `WI-M6-004b` fixed what it has to carry by a codec rather than by docs/11's sketch, and `WI-M6-003`/`003b` closed the other half: `derive_link_secret` and `derive_link_id` exist, and a Link now lands with its secret in one call
