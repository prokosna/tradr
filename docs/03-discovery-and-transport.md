...
**A hostname that cannot be read or does not validate leaves the field absent, and the advertisement still goes out.** `DisplayName` refuses an empty string, a control character and a character that reorders what renders ([below](#a-name-a-person-reads-may-not-reorder-itself-dcr-146)), and an OS that answers no hostname at all is a case this cannot fix; in all three the record is built without `n`, which is exactly the record every build before this one emitted. **A device that cannot name itself must still be discoverable** -- a missing name costs a person a label they can read, and a refused advertisement costs them the transfer.

**On Android the name is the device name its owner set, decided 2026-09-26 by DCR-160, because Android's hostname is `localhost` on every phone.** Run F showed a phone on two peer lists as `localhost`, which is the hostname rule working as written and saying nothing. The rule's intent -- publish what the device is already called, invent nothing -- is kept by reading the name the platform already shows for the phone: `Settings.Global.DEVICE_NAME` (the name under About phone), and `Build.MODEL` where that answers nothing. It is truncated to `DISPLAY_NAME_MAX_LEN` on a character boundary like a hostname, but **never split at a dot**, since a dot in a device name is part of the name and not a domain. Only Kotlin can read either value.

- **`localhost` is never a name, on any platform.** A hostname whose first label is `localhost`, compared ASCII-case-insensitively, leaves `n` absent exactly as an unreadable hostname does
- **The name is decided once, when the plugin starts, and held as the plugin's state.** The advertisement, an Invite and a `LinkReply` all read that one value, on the desktop as on Android, rather than each asking the operating system; the CLI, which never runs on Android, keeps reading the hostname directly
...
#### The advertiser is driven by a task of its own, and the schedule is what "in turn" was missing

**`start` replaces rather than fails, so rotation needed no method; what it did need was an interval, and this document gave "advertises them in rotation" no number at all.** DCR-109 settles it: **one secret per four-second slot, in the set's order**, and the whole of the rotation is a tick that returns how long to wait before the next one. The decision is in the tick and the wait is not: `Clock` reads time without being able to wait for it and `tradr-discovery` holds no runtime, so the tick answers a `Duration` and the composition root's task is the only thing that sleeps. **That is what makes the schedule testable with no radio and no `sleep`**, which rule E3 requires of every test here.

**Four seconds is read off the one measurement this design has of its own air.** `WI-M7-005c` recorded 44 reports in 90 seconds from an Android advertiser at `ADVERTISE_MODE_BALANCED` against `BluerScanner` -- about one report every two seconds against an advertising interval of 250 ms, so a scanner hears something like one advertisement in eight. **A slot has to hold several advertising intervals before it is heard at all**: at four seconds a peer expects about two reports per slot, and at one it would expect half of one and would miss whole slots routinely.

**The cycle and not the slot is what the age-out bounds.** A scanner reports only an advertisement whose EID matched a secret *it* holds, so a peer holding one of this device's secrets sees this device during that secret's slot and at no other time -- and `BleSource` declares a handle `Lost` after 30 seconds of silence. **The whole cycle therefore has to fit inside `BLE_OBSERVATION_TTL_SECS` with a slot to spare**, which at a four-second slot is six secrets -- seven would leave two seconds between the last report a peer can hear and the deadline that drops it, which is not a margin. **Past six the condition is reported once rather than hidden and the whole set still goes on the air**: shortening the slot to fit would make every peer intermittent instead of the ones past the bound, and dropping the excess would make a linked peer undiscoverable rather than slow to appear. The exit is a longer age-out or a second advertising set, and it is a Deferred entry rather than a value tuned until the symptom moves.

**`start` is called when the payload changes and not once per slot.** A device holding one secret is the ordinary case, and its payload changes only when the EID window rolls over -- so the radio is restarted four times an hour rather than nine hundred, and on Android, where `start` is a stop followed by a start, that difference is most of what the design costs. The tick still runs every slot, because the set and the window are both read at the moment of use and neither announces that it changed.

**What goes on the air is not the set a scanner matches against, and [the asymmetry above](#what-that-set-is-made-of-and-the-one-asymmetry-with-advertising) is only half of it.** [docs/11](11-account-linking.md#distributing-the-account-broadcast-key)'s step 5 stops the bootstrap EID once an ABK exists, so the advertised set is the ABK and every Link Secret, with **the bootstrap secret taking the ABK's place when there is no ABK** rather than joining it. `BroadcastSecrets` answers both questions, because the two sets are assembled from the same three stores and a second provider would be a second definition of what this device holds. **The advertised set is read per slot for the reason the matching set is read per advertisement**: a sign-in, a link, or a first ABK handover has to reach the air without a restart.

**An empty advertised set stops the radio rather than advertising nothing.** A device that is not signed in and holds no Link has no EID anyone could match, and an advertisement carrying a placeholder would be a fixed byte string on the air, which is the one thing [the EID exists to prevent](#2-ble--proximity-no-network-required-tier-0). The tick goes on running, so signing in reaches the air within a slot.

**`Unsupported` ends the task and every other error leaves it running.** That is D4's retreat on the advertising side, stated once here rather than decided four times in the implementations: a platform that cannot perform the peripheral role says so once and the device is scan-only from then on, while a busy adapter, a refused permission and an adapter that is switched off are all conditions that clear. **A cause is reported when it changes**, on `DeviceBroadcastSecrets`'s own discipline, because a slot is four seconds and a line per slot is a log nobody reads.
...
**The fallback is a decision and a mechanism, and they are two functions rather than one, 2026-09-18 by DCR-131.** Which two addresses a bind tries -- the fixed 21820 and the ephemeral 0 -- is answered in one place for both front ends, and neither number appears anywhere else; binding the pair in order, reporting the first failure before trying the second, is a mechanism that takes the pair it is handed. What a front end calls is the composition of the two, so no composition root names a port.

#### The listener binds both families, and the option is set rather than inherited (DCR-138)

**`quic_bind_addresses` answers `[::]:21820` and `[::]:0`, and the socket is built before `quinn` sees it.** Until 2026-09-19 both addresses were `0.0.0.0`, and the first time anyone sent a file between two machines it failed: the receiving host published its three global IPv6 addresses and its link-local over mDNS, the sender found it by Device ID, and every candidate was refused before a packet left. **DF-24 predicted exactly this on 2026-08-31 and its trigger read "M7 at the latest".** M7 closed without it.

**The advertisement was never wrong.** `enable_addr_auto` publishes the addresses the host holds, and the host held them; what was wrong is that the listener served one family of the two it advertised. **A peer that is discoverable and unreachable is worse than one that is absent**, because the failure arrives after discovery has already succeeded and names neither.

**`IPV6_V6ONLY` is set explicitly, and that is the whole of the cost.** `quinn::Endpoint::server` calls `std::net::UdpSocket::bind`, which inherits the platform default -- 0 on Linux, where a `[::]` socket receives IPv4 as `::ffff:a.b.c.d`, and 1 on Windows, where the same bind would stop receiving IPv4 altogether. So the socket is constructed with `socket2`, `set_only_v6(false)` is called on it, and it reaches `quinn::Endpoint::new` as a prepared socket rather than as an address. **Inheriting it would make this repair correct on the platform it was measured on and silently wrong on another**, which is the shape of the defect being repaired.

**Nothing on the dialling side changes, and that was read rather than assumed.** `quinn`'s `connect` refuses an IPv6 remote only when the endpoint is not IPv6, and maps a `SocketAddr::V4` to `::ffff:` itself when it is -- `endpoint.rs`'s `ensure_ipv6`. So `first_dialable`'s rule, that an IPv6-bound local dials either family, already describes a dual-stack endpoint and keeps its meaning. **The family filter DCR-062 added stops discarding anything**, which is what it was always going to do once the bind stopped being the narrower half.

**The split is what makes this paragraph checkable, and until it existed nothing checked it.** A bind that went straight to the ephemeral port and never tried the default left every test in the workspace green, and a device that never tries 21820 is one no Static Peer can dial by the fixed number this section promises -- there is no mDNS on a tailnet to carry the port it chose instead. The mechanism takes an address pair, so a test drives it over two ports it owns: one free, where the first address must be the one bound, and one already held, where the second must be.
...
**A device is not one of its own peers, and DCR-144 makes that a rule on 2026-09-21 because a person acted on its absence.** A device's own mDNS advertisement is answered by its own browser, so it arrives as an observation like any other and becomes a peer like any other. **Run A is what that cost**: the MacBook's list held two entries and the file went to the second, which was the MacBook, and every layer reported success because the transfer succeeded. **There is no error to find anywhere in that**, which is why the rule is here rather than in whatever draws the list.

**The rule is applied where the sources are drained and not where the list is drawn.** `drain_peer_sources` discards an `Observed` whose observation carries this device's own Device ID, so self never enters the list at all -- **and therefore cannot be listed, cannot be resolved by `resolve_peer`, and cannot be dialled by a caller who pastes its key**. Filtering in the projection would hide it from one front end's list while leaving every other path to it open, which is a smaller fix to a defect whose whole character is that nothing downstream objects.

**Discarding the arrival is safe rather than merely convenient**: `PeerList::apply` makes a `Lost` for an observation it does not hold a no-op, so a filtered peer's later disappearance needs no matching state. The list itself is unchanged and still knows nothing about which device is running it -- **the identity stays in the composition, which is the only layer that has one**, and `tradr-core` keeps merging whatever it is given.

**An unidentified observation is not filtered, and cannot be.** The Device ID is what says an advertisement is ours, so a source that has not yet supplied one is a peer until it does; mDNS always carries it in the TXT record, which is the case that matters. A Static Peer entry naming one of this device's own addresses stays visible until its first connection pins it, and pinning it to our own Device ID is a separate question nobody has asked yet.
...
**`Incoming::accept` reports the listening side's own state, and on this transport the thing that fails is one central.** A caller reads `Closed` as the listening having ended and any other error as the listening having failed, and this section said that was exactly right on `direct-quic`, where what breaks is the endpoint every peer shares. **That half of the sentence was false, and DCR-130 withdraws it**: on `direct-quic` too, the endpoint's own failure is one event and everything after it belongs to one connection, so what follows here is the trait's rule rather than this transport's -- [stated once for every transport below](#an-error-from-accept-is-the-listeners-and-a-failure-belonging-to-one-peer-never-becomes-one). Here the commonest failure is also the most harmless: a central that subscribed and walked out of range ends its byte stream between records, which is `Closed` by the rule above. **A peripheral that reported it would stop listening for the life of the process the first time anyone wandered off**, and it would be reporting the correct error about the wrong subject.
...
- **Opaque is not unchecked.** A candidate can arrive from a Brokr, which [docs/05](05-security.md#threat-model) does not trust, and it reaches logs and the UI on its way to a transport. So the core rejects an empty address, one carrying control characters, and one carrying a character that reorders what renders (DCR-146, below). It checks nothing else, because everything else is syntax only a transport knows. **The transport that receives a candidate validates it before use**, and that is a contract on each implementation rather than something the core can do for them
...
#### A name a person reads may not reorder itself (DCR-146)

**Every peer-supplied string a front end renders passes through one of three constructors, and all three refuse the same characters a filename refuses**: `Candidate::new` for an address, `DisplayName::new` for a published name, a sender label or a note, and `ObservationKey::new` for what a source calls a peer. The set is [docs/04](04-protocol.md#why-a-filename-may-not-reorder-itself)'s, exactly: `U+202A` to `U+202E`, `U+2066` to `U+2069`, `U+2028` and `U+2029`. **It is one predicate in `tradr-core`, shared with `RelPath`**, so a filename and a name cannot drift apart on what "reorders" means.

**The reason is docs/04's reason one prompt earlier.** A person picks a peer from a list and accepts a transfer from a sender label, and `char::is_control` is Unicode category `Cc` only, so `U+202E` passed all three constructors -- a peer on the LAN could advertise a TXT `n` rendering as another peer's name, and an address reading as another host. Both were dormant until DCR-145 populated `n` and the two front ends began listing addresses, which is when DF-12's own condition fired.

**What is refused is what reorders, and nothing wider.** `U+200E` and `U+200F` stay permitted for docs/04's reason. **Zero-width and look-alike characters are not refused either**, because no character list closes them: `haruka` and a Cyrillic `hаruka` differ by a letter no rule here can ban. What decides trust is the Device ID shown beside every name, and a name is a label for finding it rather than a claim about it.

**A refusal costs the string and never the peer.** A TXT record whose `n` is refused is already refused whole by the mDNS parse, and every other carrier -- a `Hello`, an Invite, a Static Peer label, an Offer's labels -- already drops a name that fails `DisplayName::new` and keeps the rest; an mDNS instance name that fails `ObservationKey::new` was already skipped the same way. **An address is the one string refused outright**: a Static Peer endpoint carrying one of these characters is refused at `add` and at load, as one carrying a control character already is -- and IDNA disallows all of them in a hostname, so no endpoint that could ever be dialled is refused by this. Nothing new is added to any of those paths.
...
### Phase 2 reads a byte count the operation must hold before it dials, and a count nobody holds yet drops `ble-gatt`

**The prefilter had no caller until DCR-111, and until DCR-108 that cost nothing.** `prefilter` and `BLE_GATT_MAX_TRANSFER_BYTES` were written beside the weight table and tested beside it, and the rule they carry -- a transfer past 512 KiB does not race a BLE candidate -- dropped a candidate no dial could reach: the composition root held one `direct-quic` transport, so a `ble-gatt` candidate was a string handed to a parser that refused it. **Making that candidate dialable turned an uncalled function into a missing rule**, which is [ADR-0002](adr/0002-ble-for-discovery-and-small-payloads.md)'s bound going unenforced at the one place able to enforce it: a peer reachable only over BLE was dialled whatever the transfer weighed.

**Phase 2 runs where the candidate is picked, because that is the only place that picks one.** The prefilter is applied and then the weight table, in the order the phases are numbered. It is composed at that site rather than folded into the set's own "can this device dial it" answer, because the two decide different questions -- one is a fact about this transfer and the other a fact about this device -- and because a rule folded into the set is a rule a second caller of the set does not get.

**The byte count belongs to the operation, and an operation either holds it before it dials or does not hold it at all.** A send counts the Items it is about to offer, and counting them means resolving each name into a root and a relative path and asking the `Vfs` for a size -- work the send already performs, after the handshake, while the Offer is built. It moves ahead of the dial, and it is performed in one place: two functions that turn a name into a `(RootId, RelPath)` are two places that can disagree about which file was meant, and only one of them would be the one that counted.

**A size nobody knows before the path is established drops `ble-gatt` rather than reading as zero.** Four operations dial a peer and only one of them counts bytes. A directory listing moves no file bytes and is bounded by its own page limit, so it keeps every candidate, and a link exchange keeps it for a stronger reason than size: [ADR-0002](adr/0002-ble-for-discovery-and-small-payloads.md) gives BLE discovery **and authentication**, so a rule that dropped the radio from the dial that authenticates would remove the one thing that ADR is for. A download names a path whose size only the peer's own listing knows, and calling that zero would commit a file of any size to a radio whose whole budget is 512 KiB. So what Phase 2 reads is a value with two cases rather than a number with a sentinel, and the unknown case is refused the radio for the same reason an oversized transfer is: the rule exists to keep a transfer off a path that cannot carry it, and "not measured" is not "small".

**Where Phase 2 empties the list, the refusal says which of the three things happened.** No candidate at all is a fact about the peer; no candidate this device can dial is a fact about this device; every candidate dropped by Phase 2 is a fact about this transfer. One message for all three makes a transfer too large for the only path a peer offers look like a peer that is not there, which is the distinction the section above already draws for the first two. **Phase 2's own answer for this case is to queue the transfer as "waiting for network", and no queue is built**, so the third case is a refusal that names the size which caused it -- and a refusal naming the cause is what a queue would later replace, rather than something a queue makes wrong.

### The set of transports a device holds is keyed by `TransportId`, and a candidate no member can dial is skipped

**"Hands each candidate to the transport that produced it" needs something that holds the transports, and until DCR-108 nothing did.** The composition root managed one `QuicTransport` by its concrete type, so the only candidate a dial could reach was a `direct-quic` one, and a `ble-gatt` candidate was a string handed to a parser that answers `invalid socket address`. **The set lives in `tradr-transport` for the same reason the weight table does** -- Phase 1 reads a peer's candidates and Phase 3 dials them, so both ends of path selection are already there -- and it is keyed rather than listed, because a candidate names exactly one transport and a set holding two answers to one candidate would have to choose between them by a rule nothing states.

**A candidate naming a transport the set does not hold is skipped rather than refused.** Which transports a device holds is a fact about the device: Linux holds a `ble-gatt` central and no peripheral, Windows holds neither half, and `relay` is M8's. A candidate this device cannot dial is a way of reaching the peer that this device does not have, which is the same situation as the peer never having offered it -- and failing the dial on that candidate instead would let one unreachable path hide every reachable one behind it.

**Where every candidate is skipped, the refusal says which of the two things happened.** "No candidate at all" is a fact about the peer and "no candidate this device can dial" is a fact about this device, and one message for both makes a missing BLE central look like a peer that is not there. It is the distinction `Unsupported` against `Unreachable` already draws inside a transport, drawn again one layer up where what reads it is a person.

### Until Phase 3 races, the dial is one candidate and the weight table picks it

**Phase 3 is not built, and a single dial stands in its place.** What that owes is not a race but agreement with one: the candidate dialled is the one a race would most likely adopt, which is the highest `class_weight` among the candidates this device can dial. **The rtt term is absent because nothing has been dialled** -- Phase 4 scores what established, and a score taken before a dial has no measurement to subtract. Ties keep the peer list's own order, fixed by transport and then by address, so the same peer produces the same dial twice.

**This replaces a hard-coded preference for `direct-quic`, and that preference was the weight table written a second time.** A policy held once as a table in `tradr-transport` and once as an `if` in the composition root has two places to disagree, and the `if` was the one that could not see `ble-gatt` at all.
...
**What decides which variant a dial gets is the source of the observation being dialled, and the branches are not interchangeable.** An observation carrying a Device ID -- mDNS, and a Static Peer entry after its first connection -- is `Device`. An observation from the Static Peer registry without one is whatever [the registry says](#the-pin-what-fills-it-in-and-what-may-never-overwrite-it), which is the Critical Module that section names. **A BLE observation is `Unpinned` and has nothing to pin**: a pin is a field of a Static Peer entry, a BLE handle has no entry and rotates away inside the window, so a `ble-gatt` first connection writes nothing back and the connection after it is a first connection too. **Any other unidentified observation is refused rather than dialled under `Unpinned`, and that is a refusal to decide rather than a judgement**: a Brokr's observations are M8's and [docs/05](05-security.md#threat-model) does not trust the source that produces them, so what a fourth source's dial expects is that source's row to argue and not this one's default.
...
### An error from `accept` is the listener's, and a failure belonging to one peer never becomes one

**Every variant above is a verdict about one peer, and `Incoming::accept` is the one signature where that is the wrong subject.** A caller reads `Closed` from `accept` as the listening having ended and anything else as the listening having failed, which is what lets the accept-handshake-serve loop terminate at all. DCR-102 established that a `ble-gatt` link's failure is not the listener's and exempted `direct-quic`, on the grounds that what breaks there is the endpoint every peer shares. **The exemption was wrong, and it was measured rather than reasoned about**: `QuicIncoming::accept` awaits two things, and only the first of them is the endpoint's. An endpoint with nothing more to offer yields no connection at all -- quinn says so with `None`, which is the `Closed` above -- and every failure after that point is one inbound connection's handshake or one peer's certificate.

**What made a latent defect a visible one is that a front end stopped outliving it.** While the loop ran as a background task inside a window that stays open, one stranger's failed handshake ended the listening and the window said nothing; `tradr receive` is a foreground command that ends when the loop ends, so the same line returns a person to their shell mid-session. **And the worse half is quieter than that**: `ApplicationClosed`, `Reset` and `LocallyClosed` are all reachable from one peer abandoning its own handshake and all three map to `Closed`, so a stranger could end this device's listening with an exit code of success and no diagnostic anywhere.

**The repair belongs to the transport and cannot belong to the loop, because the loop is reading a value that does not carry the subject.** `TransportError` is a closed set of six that names what happened and never what it happened to; only the implementation holding the endpoint knows whether the thing that failed was shared by every peer or was one connection. A loop that discarded every error would discard a listener's own failure and spin on it, and a loop that discarded some of them would be guessing from a value that says nothing either way.

**So the rule is the trait's: `accept` resolves with a channel, or with the listening having ended, and never with one peer's failure.** An implementation discards such a failure, reports it where it discards it, and waits for the next arrival. The listening ends by the listening half being dropped, or by an error that is genuinely the listener's -- which on `direct-quic` is the closed endpoint and on `ble-gatt` is a GATT server that only ever fails at its start. `GattIncoming` already works this way, which is DCR-102 read as a general rule rather than as one transport's accommodation.

**Discarding cannot spin, which is why this invents no bound and no backoff.** Each turn of the loop waits on a new inbound connection, so it is paced by arrivals and not by failures; a peer that fails a handshake repeatedly costs what the endpoint has already accepted, which is the paragraph DCR-102 wrote about links bounded by the controller, one transport over.

**The discard is reported and never silent, and what it names is the address.** An arm that drops an error without a word is rule F6's `catch {}` written in Rust, and DF-51 records that no instrument in `ci/` can see one -- so the report is the rule rather than a courtesy. A remote address is the only subject a listener holds before a handshake finishes, it is what the peripheral's pump already names as its handle, and it is not a secret rule F4 would keep out of a log.

**Nothing about the signature changes, and that is deliberate.** `accept` keeps its `TransportError`, because a transport whose listener can fail after it has started -- a relay holding a connection to a Brokr, at M9 -- needs something to answer with, and narrowing the return type to "a channel or `Closed`" would be a change to a trait in `tradr-core` that Change Drill D10 forbids. What this rule fixes is which subject an error may be about, not the type that carries it. **The caller above it therefore stays as it is**: a loop that ends on a non-`Closed` error is correct once the error can only be the listener's, and a front end that exits non-zero on one is reporting the truth.
