...
**The covered key is not on the wire, and that is what makes the join sound rather than circular.** A `KeyBinding` names the agreement key it covers, and transmitting that name would let a peer choose it. So the receiver does not read one: it reconstructs the binding over **the static key Noise has just authenticated** and checks the signature against that. **`NotForThisAgreementKey` therefore cannot arise on a Noise path** -- the comparison is true by construction -- and a peer replaying another device's identity and genuine signature is refused by the signature instead, which is the same refusal reached one check later. The variant stays for `Hello`, where the peer does send its `agreement_pub` and the comparison is a real one. DCR-093.

**Message 1 is a bare ephemeral public key and nothing authenticates it.** That is true of every Noise pattern's first message and it is worth writing down, because the obvious test -- modify message 1, expect a refusal -- asserts a property `XX` does not have, and the only way to pass it is a check in the wrong layer. **A modified message 1 is refused where the key it carried is first used**, which is the responder computing `ee` for message 2. DCR-093.
