...
**Sending is settled 2026-09-20 by DCR-139, and it is two commands because a discovered peer has to be named.** `tradr peers` prints what discovery found and `tradr send <peer> <file>...` dials one of them; the selector is the string the first command printed, matched exactly, because `PeerInfo::key` is already the form `resolve_peer` takes and a prefix rule would be an ambiguity policy bought for a paste. **What the command had to solve that neither front end had before is that discovery is a window**: a GUI polls the sources forever and a process that has just started browsing has nothing queued, so both commands browse for a bounded five seconds and `send` reports the last refusal rather than a timeout when the deadline passes. **It binds an ephemeral port rather than the default**, since a sender never listens on what it binds and a Static Peer dialling 21820 while a `send` holds it reaches a socket nothing accepts on. See [docs/02](02-architecture.md#two-front-ends-one-device).
